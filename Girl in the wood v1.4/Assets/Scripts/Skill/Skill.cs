...
public class Skill : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!silent)
        {
            ...
            {
                if (MouseManager.ins.skillSelect == 0 && Time.time > skillTime_1)
                {
                    skillTime_1 = Time.time + 闪光冷却时间;
                    //print("skill 1");
                    count = 1;
                ...
                else if (MouseManager.ins.skillSelect == 1 && Time.time > skillTime_2 && MouseManager.ins.aiming)
                {
                    ...
                    skillTime_2 = Time.time + 火球冷却时间;
                    Instantiate(fireBallExample, transform.GetChild(0).GetChild(0).position + transform.forward * 0.5f, transform.rotation);
                }
                else if (MouseManager.ins.skillSelect == 2)
                {
                    if (skillFlag_3 && Time.time > skillTime_3 && MouseManager.ins.aiming)
                    {
                        ...
                        iceBall = Instantiate(iceBallExample, transform.GetChild(0).GetChild(0).position, transform.rotation);
                    }
                    else if (!skillFlag_3)
                    {
                        skillTime_3 = Time.time + 冰球冷却时间;
                        Destroy(iceBall);
                        skillFlag_3 = true;
                    }

                }
                else if (MouseManager.ins.skillSelect == 3)
                {

                    if (skillFlag_4 && MouseManager.ins.aiming)
                    {
